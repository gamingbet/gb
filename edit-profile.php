<?php
	...
	echo $_LANG['labels']['edit-profile']; ?>
...
<?php
	...
	if($change == true){
		...
		//-------------------------------------------------------------------------------------------------------------------------------------------
		echo('<h5>'. $_LANG[ 'labels' ][ '1st' ]. '</h5>');
		
		// Nick
		//echo('<div class="calosc">'. $_LANG[ 'labels' ][ 'nick' ]. '');
		//echo('<input type="text" name="nick" value="'. $_USER[ 'nick' ] .'"  disabled></div>');
		
		// First name
		
		echo('<label for="register_05">'. $_LANG[ 'labels' ][ 'firstname' ]. '</label> ');
		echo('<input type="text" id="register_05" value="'. $inputs['firstname'] .'" name="firstname" required>');
		// Surname
		
		echo('<label for="register_06">'. $_LANG[ 'labels' ][ 'surname' ]. '</label> ');
		echo('<input type="text" id="register_06" value="'. $inputs['surname'] .'" name="surname"  required>');
		// Age
		
		echo('<label for="register_07">'. $_LANG[ 'labels' ][ 'date' ]. '</label> ');
		echo('<input type="date" id="register_07" value="'. $inputs['age'] .'" name="age"  required>');
		// County
		
		echo('<label for="register_09">'. $_LANG[ 'labels' ][ 'country' ]. '</label> ');
		echo('<select name="country"  id="register_09">');
		...
		// City + code
		echo('<label for="register_10">'. $_LANG[ 'labels' ][ 'city-code' ]. '</label><br>');
		echo('<input type="text" id="register_10" name="register_10" value="'. $inputs['city'] .'" required> <input type="text" name="register_11" value="'. $inputs['code'] .'" required>');
		
		// Street
		echo('<label for="register_12">'. $_LANG[ 'labels' ][ 'street' ]. '</label>');
		echo('<input type="text" id="register_12" name="register_12" value="'. $inputs['street'] .'"  required>');
		
		// Mail change
		echo('<input type="checkbox" id="mail_01" name="mail_change"  value="true"');
		if( isset($_POST['mail_change']) && $_POST['mail_change'] == "true" ){
			echo(' checked');
		}
		echo('> <label for="mail_01">'. $_LANG[ 'labels' ][ 'mail-change' ]. '</label>');
		
		echo('<label for="register_04">'. $_LANG[ 'labels' ][ 'new-mail' ]. '</label>');
		
		echo('<input type="text" value="'.$inputs['new_mail'].'"  id="register_04" name="register_04"><div class="status">&nbsp;</div>
			<input type="hidden" name="old_mail" value="'. $_USER[ 'email' ] .'">'); 
		
		
		// Password change
		echo('<input type="checkbox" id="pass_01" name="pass_change" value="true"');
		if( isset($_POST['pass_change']) && $_POST['pass_change'] == "true" ){
			echo(' checked');
		}
		...
		echo('<input type="password" value=""  id="register_02" name="register_02">');
		
		echo('<label for="register_03">'. $_LANG[ 'labels' ][ 'new-repw' ]. '</label>');
		echo('<input type="password" value=""  id="register_03" name="register_03">
		');
		//-------------------------------------------------------------------------------------------------------------------------------------------
	
		echo('<h5>'. $_LANG[ 'labels' ][ '2nd' ]. '</h5>');
		
		// Newsletter
		echo('<input type="checkbox" name="register_08" value="true" id="register_08"');
		if( isset($inputs['newsletter']) && $inputs['newsletter'] == "true" ){
			echo(' checked');
		}
		echo('> <label for="register_08">'. $_LANG[ 'labels' ][ 'newsletter' ]. '</label>');
		
		// avatar
		echo('<label for="register_13">'. $_LANG[ 'labels' ][ 'avatar' ]. '</label><br>');
		echo('<input type="file" id="register_13" name="file" accept="image/jpeg,image/gif,image/png">');
		...
		if(!empty($_LANG['edit-profile']['info'])){
			echo( sprintf($_LANG[ 'edit-profile' ][ 'avatar-info' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'avatar' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'avatar' ]) );
		}
		
		
		//-------------------------------------------------------------------------------------------------------------------------------------------
		echo('<h5>'. $_LANG[ 'labels' ][ '3rd' ]. '</h5>');
		
		// Ref By
		if( $_USER['refId'] != '0' ){
			echo(''. $_LANG[ 'labels' ][ 'refBy' ]. '');
			echo('<input type="text" value="'. strtolower($l->getLoginById($_USER[ 'refId' ])) .'"  disabled><br>');
		}
		
		// Ref Our
		echo($_LANG[ 'labels' ][ 'refOur' ]);
		if( !empty( $_LANG[ 'edit-profile' ][ 'ref-info' ] ) ){
			echo( sprintf($_LANG[ 'edit-profile' ][ 'ref-info' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'ref-bonus' ]) );
		}
		echo('<input type="text" value="'. $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'general' ][ 'url' ]. '/register/'.strtolower($_USER['nick']). '" id="ref-nick"  readonly>');
		
		// Ref Count	
		if( $_USER['refCount'] != '0' ){
			echo(''. $_LANG[ 'labels' ][ 'refCount' ]. '<br>');
			echo('<input type="text" value="'. $_USER['refCount']. '" readonly>');	
		}
		...
		// Send
		echo('<input type="submit" name="edit_sub" value="'. $_LANG[ 'labels' ][ 'send-form' ]. '">');
