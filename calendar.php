		...
		$calendar.= '<td class="calendar-day"><div class="calendar-day-content">';
			...
			foreach ($dates as $key => $date) {
				...
				if($time_date == $list_day)
				{
					...
					$url = '<a href="/games/'.$date['gameId'].'">Gra</a>';
					$description = $date['team1'].' vs '.$date['team2'];
					$calendar.= '<span class="calendar-content"><span class="calendar-title">'.$description.'<span class="calendar-text">'.$url.'</span></span>';
					$calendar.= '</div>';
				}
...
{
	global $db;
	$stmt = $db->prepare('SELECT * FROM `matches` WHERE `start` BETWEEN :first_day AND :last_day');
	$stmt->bindValue(':first_day', $month_begin, PDO::PARAM_INT);
	$stmt->bindValue(':last_day', $month_end, PDO::PARAM_INT);
	$stmt->execute();
