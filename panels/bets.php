...
if( $_GLOBALS['login']['login'] == "true" ){
    if( isset($_SESSION['bets']) && !empty($_SESSION['bets']) ){
        ...
        if( $_GLOBALS[ 'bets' ][ 'bets' ] == true ){
            ...
            echo('<h4>'.$_LANG['bets']['active-bets'].'</h4>
                <form action="'.$_PAGES[ 'lang' ].'" method="post">
                <ul>
            ');
            foreach($_SESSION['bets'] as $key => $value){
                $bet = getBet($key);
                ...
                echo '<li>';
                    ...
                    echo '<span>'.$_LANG['bets']['twoj-typ'].' <i>'.$enemys[$value - 1]['fullname'].'</i></span>';
                    echo ' <a href="'.$_PAGES['lang'].'/'.$key.'-'.$value.'-delete"> <img style="vertical-align: top; float: right; max-width: 20px;" src="icon/usun.png"></a>';
                echo '</li>';
            }
            echo '<li>	
                <form method="post" action="'.$_PAGES[ 'lang' ].'">
                <label for="bet_01">Stawka</label> <input type="number" class="stawka" required="" name="bet_credits" id="bet_01" min="1" placeholder="stawka">
        ...
        echo('<p>'.$_LANG['bets']['empty'].'<br><br> Wybierz mecze, które chciałbyś obstawić, a następnie podaj stawkę.</p>');
    }
}
else{
     echo('<p>Zaloguj się, a następnie wybierz mecze, które chcesz obstawić.</p>');
}
?>