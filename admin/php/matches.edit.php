	...
	if(isset($_POST['ban']) && $_POST['ban'] == "true"){
		...
		if(empty($errors)){
			...
			while($cbet = $tables_query->fetch()) {
				...
				$tables->changeCustomerCreditsByCustomerId( Tables::ADD, (int) $course, (int) $cbet['player1Id'] );
			...
			while($cbet = $tables_query->fetch()) {
				...
				$tables->changeCustomerCreditsByCustomerId( Tables::ADD, 2*((int) $course), (int) $cbet['player1Id'] );
				$tables->changeCustomerCreditsByCustomerId( Tables::ADD, 2*((int) $course), (int) $cbet['player2Id'] );
				$db->exec('UPDATE tables SET status = "finish" WHERE id = '.(int) $cbet['id']);
			}
			// Zwróć kredyty dla:
