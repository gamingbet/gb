...
<h4 class="register">Przypomnij hasło</h4>
<?php
...
if( !empty($_PAGES[ 'type' ]) && !empty($_PAGES[ 'more' ]) )
{
	...
	else
	{
		...
		{
			echo('<p class="info">'. sprintf($_LANG[ 'forgot' ][ 'info2' ], $_PAGES[ 'more' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'hash-key' ]) .'</p>');
		}
		echo('<form action="forgot/'. $_PAGES[ 'type' ] .'/'. $_PAGES[ 'more' ] .'" method="post" style="text-align: center">
		<fieldset id="register">');
		echo('');
		if( !empty($errors) )
		{
			echo('<p class="info">'.$_LANG['forgot'][ 'errors' ].'</p>');
		...
		echo('<label for="register_02">'. $_LANG[ 'labels' ][ 'pw' ]. '</label>');
		echo('<input type="password" name="register_02" id="register_02" class="pass1" required><div class="status">&nbsp;</div><br>');
		echo('<label for="register_03">'. $_LANG[ 'labels' ][ 'repw' ]. '</label>');
		echo('<input type="password" name="register_03" id="register_03"class="pass1" required>');
		echo('<br>'. recaptcha_get_html($publickey) .'<br>');
		echo('<input type="submit" name="for_submit" value="'. $_LANG[ 'labels' ][ 'change' ]. '" class="button">');
		echo('');
		echo('</fieldset>
		</form>');
...

	{
...

		{
			...
			echo('<p class="info">'. sprintf($_LANG[ 'forgot' ][ 'info' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'hash-key' ]) .'</p>');
...

		{
			...
			echo('<p class="errors">'.$_LANG['forgot'][ 'errors' ].'</p>');
		...
		echo('<label style="text-align:center;" for="forgot_01"><h5>'. $_LANG[ 'labels' ][ 'mail' ]. '</h5></label>');

		echo('<input type="email" name="forgot_01" id="forgot_01" value="'. $inputs['01'] .'" class="empty1" required>');
		echo('<input type="submit" style="float: none; margin: 0 auto; display: inherit; margin-top: 10px" name="lab_forgot" value="'. $_LANG[ 'labels' ][ 'forgot' ]. '" class="przycisk-login">');
